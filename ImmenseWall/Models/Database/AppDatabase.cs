...
namespace ImmenseWall.Models.Database
{
    ...
    class AppDatabase : ImmenseWall.Services.ISerializable<AppDatabase>
    {
        ...
        internal List<FirewallExceptionV3> GetExceptionsForApp(ExceptionSubject fromSubject, bool guiPrompt, out Application? app)
        {
            ...
            else if (fromSubject is ExecutableSubject exeSubject)
            {
                ...
                if ((exceptions.Count > 1) && guiPrompt)
                {
                    ...
                    ImmenseWall.Services.Utils.SplitFirstLine(string.Format(CultureInfo.InvariantCulture, Resources.Messages.UnblockApp, localisedAppName), out string firstLine, out string contentLines);

                    var dialog = new ImmenseWall.Services.TaskDialogue.TaskDialog
                    {
